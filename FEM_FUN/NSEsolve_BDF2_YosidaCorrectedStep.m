...
B = Acoeff(1:NVU,NVU+2:end);

if abs(t-2*dt)<1e-8
    SchurInv = inv( B' * inv(M) * B);
end
 

y1 = A\f;
y2 = [0;-SchurInv * (g-B'*y1)];
...
% GlobalP = [y2;0];
 ...
 %%% pressure correction (from Quarteroni book)
...
% % % tic
% % % %if gamma==0
% % % %     [p,FLAG,RELRES,ITER]= pcg(@(x)schurYosida(x,B,M),rhs2,1e-10,1000 );
% % % % else
% % %      [p,FLAG,RELRES,ITER]= pcg(@(x)schurYosida(x,B,M),rhs2,1e-10,1000,PressureMassMatrix );
% % % % end
% % %  display(['Flag = ' num2str(FLAG) ', iter = ' num2str(ITER) ', time=' num2str(toc) ])
% % %  if FLAG>0
% % %      error('Schur complement solve failed')
...
display(['value of updates:' num2str(max(p-y2)) ', ' num2str(max(GlobalV1-GlobalV)) ])
end
