...
y2 = S \ (g - B'*y1);
% tic
% [y2a,FLAG,RELRES,ITER]= pcg(@(x)-schurYosida(x,B,M),(g-B'*y1),1e-10,500,PressureMassMatrix(1:end-1,1:end-1) );
...
% p = -S \ rhs2;
% % tic
% % [p2,FLAG,RELRES,ITER]= pcg(@(x)schurYosida(x,B,M),-rhs2,1e-10,500,PressureMassMatrix(1:end-1,1:end-1) );
% % display(['Flag = ' num2str(FLAG) ', iter = ' num2str(ITER) ', time=' num2str(toc) ])
% % if FLAG>0
% %     error('Schur complement solve failed')
% % end
