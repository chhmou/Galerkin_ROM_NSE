function [localmat] = GradvelFunPrevTime(xy_pts, triag_no) 
%
...
global GlobalV  GlobalP  PastV AllV
...
% the basis functions.
%   Vstart = [2*(elnode(triag_no,1:10) - 1) + 1 ] ;
   Vstart = [2*(elnode(triag_no,1:3) - 1) + 1 , 2*(elnode(triag_no,4:6) + nVert - 1) + 1 ] ;
   Vel1 = AllV([Vstart], end) ;
   Vstart = Vstart + 1 ;
   Vel2 = AllV([Vstart], end) ;
...
localmat(1,1,:) = Vel1.' * Gradx ;
localmat(1,2,:) = Vel2.' * Gradx ;
localmat(2,1,:) = Vel1.' * Grady ;
localmat(2,2,:) = Vel2.' * Grady ;
