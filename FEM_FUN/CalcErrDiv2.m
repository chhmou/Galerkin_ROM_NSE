...
for itrg=1:size(elnode,1)
   ...
   pfun_valsSV = preFunSV(xy_pts,triag_no);
   ...
   preErr1 = preErr1 + quad_wghts * ( (pfun_valsSV - pfun_vals).^2 ).' ;
   preErr2 = preErr2 + quad_wghts * ( (pfun_valsSV - pfun_vals + gamma*temp2).^2 ).' ;
   preErr3 = preErr3 + quad_wghts * ( (pfun_valsSV - pfun_vals - gamma*temp2).^2 ).' ;
   preErr4 = preErr4 + quad_wghts * ( (pfun_valsSV + pfun_vals + gamma*temp2).^2 ).' ;
...
preErr1 = sqrt(preErr1);
preErr2 = sqrt(preErr2);
preErr3 = sqrt(preErr3);
preErr4 = sqrt(preErr4);

%display([ num2str(L2norm) ', ' num2str(H1norm)  ])
