...
Acoeff =  [sig*MassMatrix + nu*StiffnessMatrix + 100 * GradDivMatrix + NLMatrix,  PressureMatrix; PressureMatrix', sparse(NPU,NPU) ];
...
tic
if gamma==0
    [y2,FLAG,RELRES,ITER]= pcg(@(x)schurYosida(x,B,A),-(g-B'*y1),1e-10,1000 );
else
    [y2,FLAG,RELRES,ITER]= pcg(@(x)schurYosida(x,B,A),-(g-B'*y1),1e-10,1000,PressureMassMatrix );
end
