function [localmat] = phiFun(xy_pts, triag_no) 
%
...
global GlobalV  GlobalP  GlobalS  GlobalG GlobalPhi
...
if strcmp(vel_bas_type, 'CtsQuad') == 1 
   ...
   Vel1 = GlobalPhi([Vstart], 1) ;
   Vstart = Vstart + 1 ;
   Vel2 = GlobalPhi([Vstart], 1) ;
   
   elseif strcmp(vel_bas_type, 'CtsQuadBub') == 1
   Vstart = [2*(elnode(triag_no,1:3) - 1) + 1 , 2*(elnode(triag_no,4:6) + nVert - 1) + 1 ] ;
   nbub = 2*(nVert + nedge + triag_no) - 1 ;
   Vstart = [Vstart , nbub] ;
   Vel1 = GlobalPhi([Vstart], 1) ;
   Vstart = Vstart + 1 ;
   Vel2 = GlobalPhi([Vstart], 1) ;
   ...
   Vstart = [2*(elnode(triag_no,1:3) - 1) + 1 ] ;
   nbub = 2*(nVert + triag_no) - 1 ;
   Vstart = [Vstart , nbub] ;
   Vel1 = GlobalPhi([Vstart], 1) ;
   Vstart = Vstart + 1 ;
   Vel2 = GlobalPhi([Vstart], 1) ;
