function [localmat] = inner_prod_Vec_x_Curl_ten1_ten1(triag_no, quad_rul, V_fun, ten1a_type, ten1b_type) 
%
...
for iq = 1:nqpts
   phix(:,iq) = squeeze(quad_wghts(iq) * Gradtrue1(:,1,iq)) ;  
   phiy(:,iq) = squeeze(quad_wghts(iq) * Gradtrue1(:,2,iq)) ;
end

mat11 = -ten1a * (phiy *diag(Vfun_vals(2,:))).';
mat12 =  ten1a * (phix *diag(Vfun_vals(2,:))).';
mat21 =  ten1a * (phiy *diag(Vfun_vals(1,:))).';
mat22 = -ten1a * (phix *diag(Vfun_vals(1,:))).';

% VDivGrad(:,:) = Gradtrue1(:,1,:) + Gradtrue1(:,2,:) ;
% 
% mat = ten1a * VDivGrad.' ;
% matSS = VDivGrad * ten1b.';
% mat1 = .5 * mat - .5 * matSS;
