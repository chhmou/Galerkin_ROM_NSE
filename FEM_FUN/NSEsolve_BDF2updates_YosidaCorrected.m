...
Acoeff = Acoeff .* (abs(Acoeff)>1e-16);
 ...
 %%% Yosida with pressure correction (from Quarteroni book)
...
%%% direct solver
SchurMat = [M,B;B',C];
SchurRHS = [0*rhs1;-rhs2];
y2b = SchurMat \ SchurRHS;
p = y2b(NVU+1:end);

GlobalPupdate = p;
GlobalVupdate = y1 - ( A \ (B*p));

PresCorrection=max(abs(p-y2));

display(['Correction size:' num2str(PresCorrection)])
